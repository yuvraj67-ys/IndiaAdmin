    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-orange-400 to-orange-600 p-6 rounded-2xl shadow-lg shadow-orange-200 text-white relative overflow-hidden">
          ...
          <p className="text-xs font-bold text-orange-100">PREMIUM VEHICLES</p>
        ...
        {categories.filter(c => c.isActive).slice(0, 2).map(cat => {
          const count = vehicles.filter(v => v.category === cat.value).length;
          return (
            <div key={cat.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
              <div className="absolute -right-4 -top-4 text-6xl opacity-10">📦</div>
              <p className="text-xs font-bold text-gray-400">{cat.name.toUpperCase()}</p>
              <h3 className="text-4xl font-extrabold text-blue-600 mt-2">{count}</h3>
            </div>
          );
        })}
