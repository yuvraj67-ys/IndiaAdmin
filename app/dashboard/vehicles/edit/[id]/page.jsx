...
export default function EditVehicle({ params }) {
  ...
  const [form, setForm] = useState({ name: '', cheatCode: '', category: '', isPremium: false, isVisible: true, imageUrl: '' });
  ...
  useEffect(() => {
    Promise.all([
      ...
      const vehicle = vehData.data.find(v => v.id === id);
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={submit} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4">
        ...
        <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
          className="border p-3 rounded-xl bg-gray-50 focus:outline-none focus:border-orange-500 font-bold" />

        <label className="font-bold text-gray-700 mt-2">Cheat Code</label>
        <input type="text" required value={form.cheatCode} onChange={e => setForm({ ...form, cheatCode: e.target.value })}
          className="border p-3 rounded-xl bg-gray-50 focus:outline-none focus:border-orange-500 font-mono text-lg text-orange-700" />

        <label className="font-bold text-gray-700 mt-2">Category</label>
        <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}
          className="border p-3 rounded-xl bg-gray-50 focus:outline-none font-bold">
